...
@pytest.mark.asyncio
@pytest.mark.skipif(sys.platform == "win32", reason="Single quote escaping test only applies to Unix shells")
async def test_execute_command_with_single_quotes(
    command_executor, temp_dir
) -> None:
    ...
    # Execute the command
    result: CommandResult = await command_executor.execute_command(command, cwd=temp_dir)
...
@pytest.mark.asyncio
@pytest.mark.skipif(sys.platform == "win32", reason="Single quote escaping test only applies to Unix shells")
async def test_execute_command_with_complex_quotes(
    command_executor, temp_dir
) -> None:
    ...
    for cmd in commands:
        # Execute the command
        result: CommandResult = await command_executor.execute_command(cmd, cwd=temp_dir)
